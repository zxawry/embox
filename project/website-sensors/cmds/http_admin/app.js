...
app.controller('DemoCtrl', ['$scope', '$http', function($scope, $http) {
    ...
    var eventCallback = function (msg) {
        $scope.$apply(function () {
            $scope.sensors = JSON.parse(msg.data).map(function(sensor_val, index) {
			    return {
                	index: index,
                	val: sensor_val,
            	};
        ...
        //console.log('sensors data: ' + $scope.sensors[2].val);
    }

    var source = new EventSource('/cgi-bin/get_sensors_data');
